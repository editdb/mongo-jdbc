...
public class BasicTest extends Base {
    ...
    public void test1()
        ...
        MongoConnection conn = new MongoConnection(_db);
        MongoStatement stmt = (MongoStatement)conn.createStatement();
...
//        assertEquals( c.find( new BasicDBObject( "x" , 3 ) , ab ).toArray() , new Executor( _db , "select a,b from " + name + " where x=3" ).query().toArray() );
        ...
        assertEquals( c.find().toArray() , new Executor( stmt , "select * from " + name ).query().toArray() );
        assertEquals( c.find( empty , ab ).toArray(), new Executor( stmt , "select a,b from " + name ).query().toArray() );
        assertEquals( c.find( new BasicDBObject( "x" , 3 ) , ab ).toArray() , new Executor( stmt , "select a,b from " + name + " where x=3" ).query().toArray() );
