...
//	public static final String HTML_NO_ENTRY_X = "&#186";

    // members

    /*nima: final*/ DBCursor _cursor;
    final FieldLookup _fields = new FieldLookup();
    DBObject _cur;
    int _row = 0;
    boolean _closed = false;
    private String _noColumnValue = null; // The string value to indicate that the column is not present
    ...
    MongoResultSet( DBCursor cursor, String noColumnValue ){
        _cursor = cursor;
        _fields.init( cursor.getKeysWanted() );
        _noColumnValue = noColumnValue;
    }
    ...
    public ResultSetMetaData getMetaData(){
    	return new MongoResultSetMetaData(this);
    }
    ...
    public String getString(String columnName){
        Object x = _cur.get( columnName );
        if ( x == null ) {
            if (_cur.containsField(columnName)) {
            	return null;
            } else {
//            	return MongoResultSet.HTML_NO_ENTRY;
            	return _noColumnValue;
            }
        }
        if (x instanceof java.util.Date) {
        	java.util.Date date = (java.util.Date)x;
        	return new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(date).replace("00:00:00.000", "");
        } else {
        	return x.toString();
        }
    }
    ...
    class FieldLookup {
        ...
        String get( int i ){
            ...
            if ( s != null )
                return s;
            //nima: return null instead. Different rows will have different columns 
...
/** nima: disposes of the resultset */
